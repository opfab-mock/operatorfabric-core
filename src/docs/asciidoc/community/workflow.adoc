...
//TODO Ask if we can use images from blog, otherwise make our own
...
image::main_branches.png[Main branches, 300, float="right",align="center"]
...
image::feature_branches.png[Feature branches, 150, float="right",align="center"]
...
The repository version in `master` is `1.3.0.RELEASE`, and the `develop` branch has just been branched off it. A single
commit has been added to `develop` to change the repository version to `SNAPSHOT`.
